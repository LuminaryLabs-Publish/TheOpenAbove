...
function fabricTexture(size = 128) {
  ...
  for (let i = 0; i < size; i += 3) {
    context.fillStyle = i % 6 === 0 ? "rgb(178,178,178)" : "rgb(78,78,78)";
...
export function installEnvelopeFabricMaterials(balloon) {
    ...
    for (const material of list) {
      ...
      material.onBeforeCompile = (shader) => {
        ...
        shader.vertexShader = shader.vertexShader.replace(
          ...
          `#include <begin_vertex>
           float fabricWave = sin(position.y * 4.2 + position.x * 2.1 + uFabricTime * 0.8) * 0.008;
           transformed += objectNormal * fabricWave;`
        );
        shaderStates.push(shader);
      };
      material.customProgramCacheKey = () => "open-above-fabric-v1";
  ...
  function update(elapsed, burnerHeat = 0.18) {
    ...
    for (const material of materials) {
      ...
      material.emissiveIntensity = warmth * 0.24;
    }
  }
