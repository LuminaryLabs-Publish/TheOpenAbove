...
There is no canonical PRODUCT_COPY source object yet.
...
There is no fixture row proving README, package, campaign, runtime, HUD, and GameHost are all describing the same product.
```
...
There is no fixture matrix that tells the next implementation which acceptance rows must pass before broader extraction.
```
