...
TheOpenAbove was selected as the oldest eligible documented fallback and central-ledger catch-up target.
```
...
planned source kits:
  provide product copy authority, source fingerprints, source snapshots, acceptance rows, source consumer ledger, GameHost source readback, DOM-free source fixture proof, and central ledger parity.
```
...
The central ledger had lagged repo-local .agent state, so this pass records an explicit catch-up marker.
```
