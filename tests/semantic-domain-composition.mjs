...
const domains = {
  ...
  imageCapture: "src/domains/image-capture/image-capture-domain.js",
  experience: "src/domains/experience/experience-domain.js"
};

for (const [name, path] of Object.entries(domains)) {
  const source = readFileSync(path, "utf8");
  const semanticName = name === "imageCapture" ? "image-capture" : name;
  assert.match(source, new RegExp(`open-above-${semanticName}`));
}
...
assert.doesNotMatch(scene, /createAirMailDomain/);
